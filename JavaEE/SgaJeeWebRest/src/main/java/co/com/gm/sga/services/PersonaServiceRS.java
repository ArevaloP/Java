...
public class PersonaServiceRS {
    ...
    @GET
    @Produces({MediaType.APPLICATION_XML, MediaType.APPLICATION_JSON})
    @Path("{id}")//Hace referencia al Path de personas/algunParam, en este caso id
    public Persona econtrarPersonaPorId(@PathParam("id") int id) {
        return personaService.encontarPersonaId(new Persona(id));
    }
    ...
    public Response modificarPersona(@PathParam("id") int id, Persona personaModificada) {
        try {
            Persona persona = personaService.encontarPersonaId(new Persona(id));
            if (persona != null) {
